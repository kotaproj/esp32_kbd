...
#if 1
    // for up/down/left/right/center
    JsCode_e eJsCode;
    if (x < 500)
    {
        eJsCode = JS_CODE_LEFT;
    }
    else if (x > 3600)
    {
        eJsCode = JS_CODE_RIGHT;
    }
    else
    {
        if (y < 500)
        {
            eJsCode = JS_CODE_DOWN;
        }
        else if (y > 3600)
        {
            eJsCode = JS_CODE_UP;
        ...
        {
            eJsCode = JS_CODE_CENTER;
        }
    }

    // 状態の判定
    if (s_xBakJsCrd.eJsCode != eJsCode)
    {
#if DEBUG_API_JS
        Serial.printf("%s - eJsCode:%d, x:%d, y:%d\n", __func__, eJsCode, x, y);
#endif
        s_xBakJsCrd.eJsCode = eJsCode;

        JsMessage_t xMessage;

        xMessage.ulCode = (uint32_t)eJsCode;
        xMessage.slAxisX = x;
        xMessage.slAxisY = y;
